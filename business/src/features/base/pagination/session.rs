...
impl PaginatorSession {
    ...
    pub async fn get_page(
        ...
        self.cache_repo
            .get_cached_ids(&self.prefix_key, &pagination_id.to_string(), limit, offset)
            .await
    }

    /// Cache a list of UUIDs for a new session.
    pub async fn cache_pagination(
        &self,
        pagination_id: &Uuid,
        ids: &[Uuid],
    ) -> Result<(), CacheRepositoryError> {
        self.cache_repo
            .cache_ids(&self.prefix_key, &pagination_id.to_string(), ids)
            .await
    }
    ...
    pub async fn get_all_ids(
        ...
        self.cache_repo
            .get_all_cached_ids(&self.prefix_key, &pagination_id.to_string())
            .await
    }
}